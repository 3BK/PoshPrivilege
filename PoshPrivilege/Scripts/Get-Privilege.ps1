Function Get-Privilege {
    ...
    Param (
        [parameter(ParameterSetName='Default')] [Privileges[]] $Privilege, 
		[parameter(ParameterSetName='Default')] [string] $Computername = $Env:Computername,
        [parameter(ParameterSetName='CurrentUser')] [switch]$CurrentUser
    )
    Switch ($PSCmdlet.ParameterSetName) {
		...
		'CurrentUser' {
			...
			#https://docs.microsoft.com/en-us/dotnet/api/system.security.principal.tokenaccesslevels?view=dotnet-plat-ext-3.1
			...
			[void][PoShPrivilege]::OpenProcessToken( $hProcess, $TOKEN_ALL_ACCESS, [ref]$hProcessToken )
